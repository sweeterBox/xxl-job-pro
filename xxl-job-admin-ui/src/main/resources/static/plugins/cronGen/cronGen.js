(function ($) {
    ...
    $.fn.extend({
        cronGen: function (options) {
            ...
            var topMenu = $("<ul/>", { "class": "nav nav-tabs", id: "CronGenTabs", role: "tablist" });
            $('<li/>', { 'class': 'nav-item', style: "margin-left: 10px",role:"presentation" }).html($('<a id="SecondlyTab" class="active" href="#Secondly">秒</a>')).appendTo(topMenu);
            $('<li/>', { 'class': 'nav-item', style: "margin-left: 10px",role:"presentation" }).html($('<a id="MinutesTab" href="#Minutes">分钟</a>')).appendTo(topMenu);
            $('<li/>', { 'class': 'nav-item', style: "margin-left: 10px",role:"presentation" }).html($('<a id="HourlyTab" href="#Hourly">小时</a>')).appendTo(topMenu);
            $('<li/>', { 'class': 'nav-item', style: "margin-left: 10px",role:"presentation" }).html($('<a id="DailyTab" href="#Daily">日</a>')).appendTo(topMenu);
            $('<li/>', { 'class': 'nav-item', style: "margin-left: 10px",role:"presentation" }).html($('<a id="MonthlyTab" href="#Monthly">月</a>')).appendTo(topMenu);
            $('<li/>', { 'class': 'nav-item', style: "margin-left: 10px",role:"presentation" }).html($('<a id="WeeklyTab" href="#Weekly">周</a>')).appendTo(topMenu);
            $('<li/>', { 'class': 'nav-item', style: "margin-left: 10px",role:"presentation" }).html($('<a id="YearlyTab" href="#Yearly">年</a>')).appendTo(topMenu);
            ...
            var tabContent = $("<div/>", { "class": "tab-content", "style": "border:0px; margin-top:-20px; margin-left: 20px;" });
            ...
            // Add an input
            var $i = $("<input>", { type: 'text', placeholder: 'cron表达式...', name: 'cronGen_display', id: 'scheduleConf' }).addClass("form-control").val($(that).val());
            $i.appendTo($g);
            // Add the button
            ...
            $b.popover({
                ...
                template: '<div class="popover" style="max-width:500px !important; width:425px;left:250px !important;"><div class="arrow"></div><div class="popover-inner"><h3 class="popover-title"></h3><div class="popover-content"><p>'+$(cronContainer).html()+'</p></div></div></div>',
                sanitize:false,
                placement: options.direction || 'right'
    ...
    var refreshRunTime = function () {
        $.ajax({
            type : 'GET',
            url : base_url + "/v1.0/task/nextTriggerTime",
            data : {
                "scheduleType" : 'CRON',
                "scheduleConf" : $('#scheduleConf').val()
            },
            dataType : "json",
            success : function(data){
                $('#runTime').val(data.join("\n"));
            }
        });
    };
